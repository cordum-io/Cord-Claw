...
func TestCheckPromptBuildConstrainsUnicodeObfuscatedSecretPrompt(t *testing.T) {
	h := New(config.Config{CacheMaxSize: 100, CacheTTL: 5, FailMode: "closed"}, &fakeSafety{decision: cache.Decision{Decision: "ALLOW", Reason: "ok", Snapshot: "snap-1"}})

	obfuscated := "ѕк-TESTKEY-DONTLEAK"
	payload := CheckRequest{Tool: "prompt_build", Hook: "before_prompt_build", PromptText: "summarize café " + obfuscated, Agent: "agent-1", Model: "gpt-4.1-mini"}
	body, _ := json.Marshal(payload)
	...
	modified, _ := response.Constraints["modified_prompt"].(string)
	if modified != "summarize café <REDACTED-OPENAI_KEY>" {
		t.Fatalf("modified prompt did not redact only the original obfuscated span")
	}
	if strings.Contains(modified, obfuscated) || strings.Contains(modified, "sk-TESTKEY-DONTLEAK") {
		t.Fatalf("modified prompt leaked original or normalized secret")
	}
	...
	if strings.Contains(string(encodedAudit), obfuscated) || strings.Contains(string(encodedAudit), "sk-TESTKEY-DONTLEAK") {
		t.Fatalf("audit entry leaked original or normalized secret")
	}
}
